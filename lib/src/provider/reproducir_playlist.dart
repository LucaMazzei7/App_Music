...
class ReproductorProvider extends ChangeNotifier {
  ...
  // ==========================================
    // TUS CAMBIOS: Extracción Automática de Color para tus Temas
    // ==========================================
    if (imagen.isNotEmpty) {
      try {
        // Genera la paleta de colores desde la URL o Asset dinámico
        final PaletteGenerator palette = await PaletteGenerator.fromImageProvider(
          imagen.startsWith('http') ? NetworkImage(imagen) : AssetImage(imagen) as ImageProvider,
        );
        
        // 2. EXTRAE EL COLOR DOMINANTE: Guardamos el color principal
    Color? colorDetectado = palette.dominantColor?.color;

    if (colorDetectado != null) {
      // 3. Notifica a tu proveedor de temas para que cambie la UI de la app
      ThemeModifierProvider().actualizarColorSintonizado(colorDetectado); 
    }
      } catch (e) {
        print("No se pudo extraer el color de la carátula: $e");
      }
    }
    // ==========================================
    try {
